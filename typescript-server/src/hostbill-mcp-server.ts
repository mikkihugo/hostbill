...
export class HostBillMCPServer {
    ...
    private hostbill: HostBillAPIClient;
    private tools = new Map<string, MCPTool>();
    private discoveredMethods: string[] = [];
    private maxTools = 50; // Switch to meta-tools pattern if exceeded
     ...
     */
    private async registerIndividualTools(): Promise<void> {
        for (const method of this.discoveredMethods) {
            ...
            this.tools.set(toolName, {
                ...
                handler: async (args: any) => this.executeAPIMethod(method, args)
            });
        }
    ...
    private registerMetaTools(): void {
        ...
        this.tools.set('hostbill_list_methods', {
            ...
            handler: async (args: any) => this.listAPIMethods(args)
        });
        ...
        this.tools.set('hostbill_get_method_details', {
            ...
            handler: async (args: any) => this.getMethodDetails(args)
        });
        ...
        this.tools.set('hostbill_call_api', {
            ...
            handler: async (args: any) => this.callAPIMethod(args)
        });
     ...
     */
    private async handleToolCall(id?: number | string, params: any = {}): Promise<MCPResponse> {
     ...
     */
    private async executeAPIMethod(method: string, args: any): Promise<string> {
        try {
     ...
     */
    private async listAPIMethods(args: any): Promise<string> {
     ...
     * Get method details
     */
    private async getMethodDetails(args: any): Promise<string> {
        const { method } = args;
     ...
     * Call API method
     */
    private async callAPIMethod(args: any): Promise<string> {
     ...
     */
    private generateToolName(method: string): string {
     ...
     */
    private async generateToolSchema(method: string): Promise<{ description: string; inputSchema: any }> {
        try {
            const details = await this.hostbill.getMethodDetails(method);
     ...
     */
    private generateInputProperties(parameters: any[]): any {
     ...
     */
    private sendError(message: string, id?: number | string): void {
        const error = {
            jsonrpc: '2.0',
            id,
            error: {
